"""Enclosure for the Hörmann HCP2 adapter carrier PCB in ../pcb.

Two parts: `bottom`, a tray, and `top`, a lid with a hexagonal ventilation
pattern fastened to four corner columns. It is not waterproof, so the lid rests
flat on the wall rim without a gasket groove.

The only external opening accepts the 6P6C plug at J1. U1 USB-C and JP1 remain
inside and are accessed by removing the lid.

Coordinates: the base is centered in the XY plane and walls grow in +Z. The PCB
matches the KiCad render, with the J1 edge toward +Y and USB edge toward +X.
`kicad()` converts PCB coordinates (origin at upper left, y downward) to
enclosure coordinates so values from pcb.py remain directly reusable.

    uv run python enclosure/case.py           export STL files
    uv run python enclosure/case.py --show    open in the ocp_vscode viewer
    uv run python enclosure/case.py --png     render preview-*.png
"""
...
def kicad(x, y):
    """Convert KiCad PCB coordinates (upper left, y downward) to enclosure XY."""
    return pcb_x + x - pcb_length / 2, pcb_y - (y - pcb_width / 2)


# Opening for the 6P6C plug in the +Y wall, centered in front of J1. The jack
# footprint defines its base; the selected 53-6p6c vendor drawing defines height.
# The latch faces the PCB, so the plug lever runs below the body and needs space
# below the jack face.
jack_height = 11.65  # Body height above the PCB per vendor drawing
jack_hole_side = 1.0  # Clearance beside each side of the jack
jack_hole_above = 2.0  # Clearance above the jack
